...
def create_or_load_index(
    ...
    """Attempts to load an index from a file, or creates one if it doesn't,
    ...
    if not index_json_file_path.exists() or force_recreate:
        logger.info("Creating index for file")
        index = generate_index(
            haystack_file_path, index_chunk_size=index_chunk_size
        )
        logger.info(f"Index created with {len(index):,} entries")

        store_index_json(index, index_json_file_path)
        logger.info(
            f"Index stored in {index_json_file_path.name}, "
            f"size: {index_json_file_path.stat().st_size:,} bytes"
        )
